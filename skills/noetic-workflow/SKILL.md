...
| 模式 | 用户说法 | 行为 |
| --- | --- | --- |
| **delegate**（默认） | 「不用 Kanban」「委派子代理」「在当前宿主跑」 | 读 `workflow.yaml`，输出按依赖执行的子代理任务计划 |
...
7. Hermes `planned` 模式统一使用 `worker` profile 接收 Kanban task；data/gen 分工只通过 `role_skill` 和任务正文表达。
